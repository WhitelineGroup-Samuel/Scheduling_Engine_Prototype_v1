...
@pytest.mark.skipif(
    IS_NON_EMPTY_STR is None, reason="is_non_empty_str helper not implemented"
)
...
def test_is_non_empty_str_table(value: object, expected: bool) -> None:
    ...
    assert (
        result is expected
    ), f"Expected {expected} for input {value!r}, received {result}"


@pytest.mark.skipif(
    IS_VALID_EMAIL is None, reason="is_valid_email helper not implemented"
)
@pytest.mark.parametrize(
    ("value", "expected"),
    [
...
def test_is_valid_email_table(value: str, expected: bool) -> None:
    ...
    assert (
        result is expected
    ), f"Expected {expected} for email {value!r}, received {result}"
...
def test_is_valid_url_table(value: str, expected: bool) -> None:
    ...
    assert (
        result is expected
    ), f"Expected {expected} for URL {value!r}, received {result}"
